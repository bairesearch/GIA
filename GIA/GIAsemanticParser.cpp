...
bool GIAsemanticParserClass::performSemanticParserLookupAndCreateSemanticNetworkBasedUponSemanticDependencyParsedSentences(GIAtranslatorVariablesClass* translatorVariables, const string inputPlainTXTFileName, string inputTextNLPrelationXMLfileName, const string inputTextNLPfeatureXMLfileName, const string outputCFFfileName, const string NLPexeFolderArray[])
{
	...
	if(!GIAnlp.parseNLPparserFeaturesFile(inputTextNLPfeatureXMLfileName, translatorVariables->isQuery, translatorVariables->firstParagraphInList, translatorVariables->NLPfeatureParser, &createNewSentences))
	{
		result = false;
	}
	...
	if(NLPfeatureParser == GIA_NLP_PARSER_RELEX)
	{
		if(!this->lookupSemanticParserFiles(translatorVariables->firstParagraphInList, translatorVariables->NLPfeatureParser))	//finds all corresponding GIA2 semantic dependency files and parses them
		{
			...
			#else
			#ifdef GIA_SEMANTIC_PARSER_PARSE_PARTIALLY_FILLED_SEMANTIC_RELATIONS
	...
	if(!GIAnlp.parseNLPparserRelationsFile(inputTextNLPrelationXMLfileName, translatorVariables->isQuery, translatorVariables->firstParagraphInList, translatorVariables->NLPdependencyRelationsParser, translatorVariables->NLPrelexCompatibilityMode, createNewSentences, onlyParseIfCorpusLookupFailed))
	{
		result = false;
	}
	#else
...
bool GIAsemanticParserClass::lookupSemanticParserFiles(GIAsentence* firstSentenceInList, const int NLPfeatureParser)
{
	...
	while(currentSentenceInList->next != NULL)
	{
		...
		*/
		...
		#ifdef GIA_SEMANTIC_PARSER_WRITE_SEMANTIC_RELATIONS_UNOPTIMISED_TEXT_CORPUS	//prioritise text corpus defined semantic relations matching full sentence (or GIA_SEMANTIC_PARSER_SUBSETS_UNOPTIMISED_TEXT_CORPUS)
		if(!GIAsemanticParserDatabase.loadSemanticParserCorpusDatabaseFile(currentSentenceInList, currentSentenceInList->firstFeatureInList))
		{
				...
				if(stillNotFoundASubset)
				{
					...
					if(!notFoundASubsetForAtLeastTwoWords)	//NB !notFoundASubsetForAtLeastTwoWords condition is optional; it is used because syntactic relations will be used in this case regardless, see "warning: GIA2 corpus entry not found (will generate corpus entry)"
					{
						...
						if(!SHAREDvars.textInTextArray(secondWordInTupleFeature->word, semanticParserOptimiseBasedOnConjunctionsIllegalSecondWordInTupleArray, GIA_SEMANTIC_PARSER_SUBSETS_OPTIMISE_BASED_ON_CONJUNCTIONS_ILLEGAL_CENTRAL_WORD_NUMBER_OF_TYPES))
						{
							...
							{
								int GIA2semanticDependencyRelationProbabilityTotalArray[GIA_SEMANTIC_PARSER_SUBSETS_OPTIMISED_DATABASE_SEMANTIC_RELATION_NUMBER_OF_TYPES];
								GIAsemanticParserDatabase.initialiseIntArray(GIA2semanticDependencyRelationProbabilityTotalArray, GIA_SEMANTIC_PARSER_SUBSETS_OPTIMISED_DATABASE_SEMANTIC_RELATION_NUMBER_OF_TYPES, 0);
							#endif
								GIAfeature* firstFeatureInSentenceSubset = firstFeatureInSentenceSubsetInitial;
								while(firstFeatureInSentenceSubset->entityIndex <= maxIndexOfFirstWordInTuple)		//changed 2k1a for GIA_SEMANTIC_PARSER_SUBSETS_OPTIMISE_BASED_ON_CONJUNCTIONS compatibility: verify that feature entityIndices are not mutated by GIA referencing*	//OLD:	for(int firstWordInSentenceSubsetIndex=1; firstWordInSentenceSubsetIndex<=secondWordInTupleIndex-(GIA_SEMANTIC_PARSER_CONNECTIONIST_NETWORK_MIN_SUBSET_SIZE-1); firstWordInSentenceSubsetIndex++)
								{
			...
			{
				result = false;
				#ifndef GIA_SEMANTIC_PARSER_PARSE_PARTIALLY_FILLED_SEMANTIC_RELATIONS
