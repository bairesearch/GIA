...
bool GIAtxtRelTranslatorClass::executeTxtRelTranslatorWrapper(GIAtranslatorVariablesClass* translatorVariables, vector<XMLparserTag*>* GIAtxtRelTranslatorRulesTokenLayers, vector<GIAtxtRelTranslatorRulesGroupType*>* GIAtxtRelTranslatorRulesGroupTypes)
{
	...
	*/
	...
	if(!GIAtxtRelTranslatorInverseNeuralNetwork.transferParseTreePOStypeInferredToWordList(translatorVariables))	//is this required without GIA_TXT_REL_TRANSLATOR_RULES_ITERATE_OVER_UNAMBIGUOUS_POS_PERMUTATIONS_AT_START?
	{
		result = false;
	}
	#endif
...
#endif
{
	...
	GIAtxtRelTranslatorRulesGroup* firstParseTreeGroupTemp = firstParseTreeGroup;
		...
		#else	
		if(GIAtxtRelTranslatorInverseNeuralNetwork.generateParseTreeIntro(GIAtxtRelTranslatorRulesTokenLayers, GIAtxtRelTranslatorRulesGroupTypes, sentenceContents, firstParseTreeGroupTemp, &performanceTemp, parseIsolatedSubreferenceSets))
		#endif
		{
			passedTemp =  true;
			result = true;
	...
	if(result)
	{
		...
		GIAtxtRelTranslatorInverseNeuralNetwork.restoreAllWordsAlreadyFoundMatchInComponent(sentenceContents, *performance);
	}
	#endif
