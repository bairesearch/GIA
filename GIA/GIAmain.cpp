...
int main(const int argc, const char** argv)
{
	...
	#endif
	{
		...
		{
			cout << "GIA.exe - Project Version: 3d2g 14-July-2017" << endl;
			exit(EXIT_OK);
		}
...
bool GIAmainClass::executeGIA2()
{
	...
	for(int inputFileNameIndex=0; inputFileNameIndex<numberOfInputFilesInList; inputFileNameIndex++)	//CHECKTHIS: change back to 0
	{
		...
		#ifdef GIA_NEURAL_NETWORK_ACTIVE
		if(ANNuseInputTextXMLFile)
		{
			if(!GIAneuralNetworkXmlConversion.readNeuralNetXMLfile(ANNinputTextXMLFileName, ANNfirstInputNeuronInNetwork))
			{
				result = false;
			}
		}
	...
	GIAneuralNetwork.generateNeuralNetFromSemanticNet(translatorVariables);	//generate neural network
	#endif
	if(ANNdrawOutput)
	{
		string ANNoutputTALFileName = string(NEURAL_NETWORK_VISUALISATION_BASE_FILE_NAME) + NEURAL_NETWORK_VISUALISATION_TAL_FILE_EXTENSION;
	...
	if(ANNuseOutputXMLFile)
	{
		if(!GIAneuralNetworkXmlConversion.writeNeuralNetXMLfile(ANNoutputXMLFileName, translatorVariables->firstInputNeuronInNetwork))
		{
			result = false;
		}
	}		
