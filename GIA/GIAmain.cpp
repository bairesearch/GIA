...
int main(int argc,char **argv)
{
	...
	if(exists_argument(argc,argv,"-itxt") || exists_argument(argc,argv,"-irelex") || exists_argument(argc,argv,"-ixml"))
	{
		...
		if(exists_argument(argc,argv,"-iorelex"))
		{
			inputTextRelexXMLFileName=get_char_argument(argc,argv,"-iorelex");
	...
	*/
	...
	{//if relexed claims not available, then execute relex on generated claims summary (inputTextPlainTXTFileName)
		useInputTextPlainTXTFile = true;
	}
#endif	
	...
	if(useInputTextRelexXMLFile)
	{
		...
		else
		{
			//cout << "as" << endl;
			#ifdef USE_CE	
			if(!parseRelexFile(inputTextRelexXMLFileName, entityNodesCompleteList, conceptEntityNodesList, propertyEntityNodesList, actionEntityNodesList, conditionEntityNodesList, timeConditionNodesList, timeConditionNumbersList, false, firstClaimInHeirachy, claimsList))
			#else
	...
	if(useInputQueryRelexXMLFile)
	{
		...
		{
			#ifdef USE_CE	
			if(!parseRelexFile(inputQueryRelexXMLFileName, entityNodesCompleteListQuery, conceptEntityNodesListQuery, propertyEntityNodesListQuery, actionEntityNodesListQuery, conditionEntityNodesListQuery, timeConditionNodesListQuery, timeConditionNumbersListQuery, true, firstClaimInHeirachy, claimsList))
			#else
