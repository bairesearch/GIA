...
#endif
"\n\t-ionlp [string]    : NLP parser .xml intermediary input/output filename (def: inputNPL.xml)"
...
"\n\t-ionlpq [string]   : query NLP parser .xml intermediary input/output filename (def: inputNPLQuery.xml)"
...
int main(int argc,char **argv)
{
	...
	string inputTextNLPParsedXMLFileName = "inputNPL.xml";
	...
	string inputQueryNLPParsedXMLFileName = "inputNPLQuery.xml";
...
#endif
{
	...
	#ifdef USE_CE	
	if(!createSemanticNetworkBasedUponDependencyGrammarParsedSentences(firstParagraphInList, entityNodesCompleteList, conceptEntityNodesList, propertyEntityNodesList, actionEntityNodesList, conditionEntityNodesList, timeConditionNodesList, timeConditionNumbersList, isQuery, firstClaimInHeirachy, claimsList))
	#else
	if(!createSemanticNetworkBasedUponDependencyGrammarParsedSentences(firstParagraphInList, entityNodesCompleteList, conceptEntityNodesList, propertyEntityNodesList, actionEntityNodesList, conditionEntityNodesList, timeConditionNodesList, timeConditionNumbersList, isQuery))
	#endif
	{
		result = false;
	}
...
#endif
{
	...
	initialiseGIATranslatorForTexualContext();
		...
		Sentence * firstSentenceInList = firstParagraphInList->firstSentenceInList;
		#endif
		convertSentenceListRelationsIntoGIAnetworkNodesBasedUponClaimHeirachy(conceptEntityNodesList, timeConditionNodesList, timeConditionNumbersList, firstSentenceInList, firstClaimInHeirachy, claimsList);				
	#else	
		#ifdef GIA_USE_RELEX_UPDATE_ADD_PARAGRAPH_TAGS
