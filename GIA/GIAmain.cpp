...
int main(const int argc, const char** argv)
{
	...
	#ifndef LRP_PREPROCESSOR_POS_TAGGER_GENERATE_DATABASE
	...
	{
		cout << "GIA.exe - Project Version: 3m7a 11-September-2020" << endl;
		exit(EXIT_OK);
	}
...
bool GIAmainClass::executeGIA2()
{
	...
	#ifdef LRP_PREPROCESSOR_POS_TAGGER_DATABASE_PERSISTENT
	LRPpreprocessorPOStaggerDatabase.initialisePOStaggerDatabase(POStaggerDatabaseFolderName);
	#endif
	...
	for(int inputFileNameIndex=0; inputFileNameIndex<numberOfInputFilesInList; inputFileNameIndex++)	//CHECKTHIS: change back to 0
	{
		...
		{
			if(!LRPpreprocessor.preprocessTextForGIAwrapper(useLRP, &inputTextPlainTXTfileName, outputLRPTextPlainTXTFileName, false, &(translatorVariables->LRPpreprocessorTranslatorVariables), &useInputTextPlainTXTFile, inputTextNLPfeatureXMLfileName))
			{
				result = false;
			}
		}
	...
	if(useInputQuery)
	{
		...
		{
			if(!LRPpreprocessor.preprocessTextForGIAwrapper(useLRP, &inputQueryPlainTXTFileName, outputQueryLRPTextPlainTXTFileName, true, &(translatorVariablesQuery->LRPpreprocessorTranslatorVariables), &useInputQueryPlainTXTFile, inputQueryNLPfeatureXMLFileName))
			{
				result = false;
			}
		}
		...
		{
			GIAdraw.printGIAnetworkNodes(translatorVariablesQuery, rasterImageWidth, rasterImageHeight, outputQueryLDRFileName, outputQuerySVGFileName, outputQueryPPMFileName, displayInOpenGLAndOutputScreenshot, useOutputQueryLDRFile, useOutputQuerySVGFile, useOutputQueryPPMFile);
		}
	...
	if(printOutput)
	{
		GIAdraw.printGIAnetworkNodes(translatorVariables, rasterImageWidth, rasterImageHeight, outputTextLDRFileName, outputTextSVGFileName, outputTextPPMFileName, displayInOpenGLAndOutputScreenshot, useOutputTextLDRFile, useOutputTextSVGFile, useOutputTextPPMFile);
	}
