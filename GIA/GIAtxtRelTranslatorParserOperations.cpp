...
bool GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations(GIAtranslatorVariablesClass* translatorVariables, GIAtxtRelTranslatorRulesGroupParseTree* currentParseTreeGroup, GIAtxtRelTranslatorParserForwardPropogationSignalData* parserForwardPropogationSignalData, int layer)
{
	...
	{		
		...
		GIAtxtRelTranslatorSANIPropagateOperations.printComponent(parseTreeComponent, layer);
		#endif
		...
		if(parseTreeComponent->componentType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_COMPONENTTYPE_GROUP)
		{
			...
			{
				cout << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations error: (parseTreeComponent->parseTreeGroupRef == NULL)" << endl;
				exit(EXIT_ERROR);
			}
		}
...
{	
	...
	for(int i=0; i<currentParseTreeGroup->components.size(); i++)
	{
		...
		for(int w=GIA_NLP_START_ENTITY_INDEX; w<=translatorVariables->currentPreprocessorSentenceInList->sentenceContentsLRP.size(); w++)
		{
			...
			#ifdef GIA_TXT_REL_TRANSLATOR_SANI_PARSE_SIMULTANEOUS_SET_WORD_POSTYPE_INFERRED_DYNAMIC_STREAMLINED
			...
			if(parseTreeComponent->candidateStringMatch != NULL)
			{
				...
				#ifdef GIA_TXT_REL_TRANSLATOR_SANI_PARSE_SIMULTANEOUS_SET_WORD_POSTYPE_INFERRED_DYNAMIC_OLD_BUG
	...
	for(int i=0; i<currentParseTreeGroup->components.size(); i++)
	{
		...
		for(int j=0; j<GIA_TXT_REL_TRANSLATOR_MAX_NUMBER_OF_SEMANTIC_FUNCTIONS_EXECUTED_PER_GROUP; j++)
		{
			...
			//{	
				...
				{
					//cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} warning: (currentParseTreeGroup->semanticRelationFunctionName)[j] is undefined; parseTreeComponent->semanticRelationIndexType)[j] = " << (parseTreeComponent->semanticRelationIndexType)[j] << endl;
				}
			//}
		}
		...
		if(parseTreeComponent->semanticRelationReturnEntity)
		{
			...
			*/
			...
			{
				cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error; (parseTreeComponent->semanticRelationReturnEntity) && (parseTreeComponentSignalData->semanticRelationReturnEntity == NULL)" << endl;
				exit(EXIT_ERROR);
			}
		...
		if(parseTreeComponent->semanticRelationConnect)
		{
			...
			{
				cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error: (parseTreeComponent->semanticRelationConnect) && (semanticRelationRecordEntitySubject == NULL)" << endl;
				exit(EXIT_ERROR);
			}
		}
		#endif
		...
		if(parseTreeComponent->semanticRelationReturnEntity && parseTreeComponent->semanticRelationReturnFunctionName != "")	//first/lowest level layer; defines both semanticRelationReturnFunctionNameIndexType and semanticRelationReturnFunctionName
		{
			...
			{				
				cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error: (parseTreeComponent->foundExplicitReturnFunctionName != "") && (parseTreeComponent->semanticRelationReturnFunctionNameIndexType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_SEMANTIC_RELATION_INDEX_TYPE_UNKNOWN)" << endl;
				exit(EXIT_ERROR);	
			}
		}
		...
		if(specialFunctionTests)
		{
			...
			for(vector<GIAentityNode*>::iterator iter1 = (parserForwardPropogationSignalData->semanticRelationReturnFunctionEntityArray).begin(); iter1 != (parserForwardPropogationSignalData->semanticRelationReturnFunctionEntityArray).end(); )
			{
				...
				*/
				...
				{
					cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error: Illegal semanticRelationReturnFunctionNameIndexType; (tempEntity->semanticRelationReturnFunctionNameIndexType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_SEMANTIC_RELATION_INDEX_TYPE_LIST)" << endl;
					exit(EXIT_ERROR);
				}
				else if(tempEntity->semanticRelationReturnFunctionNameIndexType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_SEMANTIC_RELATION_INDEX_TYPE_SUBJECT)
				{
					if(entitySemanticRelationFunctionSubject[GIA_TXT_REL_TRANSLATOR_SEMANTIC_RELATION_RETURN_FUNCTION_NAME_INDEX_TYPE_SEMANTIC_FUNCTION_EXECUTION_INDEX] != NULL)
					{
						if(entitySemanticRelationFunctionSubject[GIA_TXT_REL_TRANSLATOR_SEMANTIC_RELATION_RETURN_FUNCTION_NAME_INDEX_TYPE_SEMANTIC_FUNCTION_EXECUTION_INDEX]->entityIndexTemp < tempEntity->entityIndexTemp)
						{
							...
							createSemanticRelationPair(translatorVariables, tempEntity->semanticRelationReturnFunctionName, entitySemanticRelationFunctionSubject[GIA_TXT_REL_TRANSLATOR_SEMANTIC_RELATION_RETURN_FUNCTION_NAME_INDEX_TYPE_SEMANTIC_FUNCTION_EXECUTION_INDEX], tempEntity, sameReferenceSet, semanticRelationReturnConnectionDynamic, layer);
				...
				{
					cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error: Illegal semanticRelationReturnFunctionNameIndexType; (tempEntity->semanticRelationReturnFunctionNameIndexType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_SEMANTIC_RELATION_INDEX_TYPE_OBJECT)" << endl;
					exit(EXIT_ERROR);				
				}
				else if(tempEntity->semanticRelationReturnFunctionNameIndexType == GIA_TXT_REL_TRANSLATOR_RULES_GROUPS_COMPONENT_SEMANTIC_RELATION_INDEX_TYPE_DELIMITER)
				{
					...
					if(entitySemanticRelationFunctionDelimiter[GIA_TXT_REL_TRANSLATOR_SEMANTIC_RELATION_RETURN_FUNCTION_NAME_INDEX_TYPE_SEMANTIC_FUNCTION_EXECUTION_INDEX] != NULL)
					{
						...
						{
							//bool sameReferenceSet = tempEntity->semanticRelationReturnFunctionNameSameReferenceSet;
							createSemanticRelationPair(translatorVariables, tempEntity->semanticRelationReturnFunctionName, entitySemanticRelationFunctionDelimiter[GIA_TXT_REL_TRANSLATOR_SEMANTIC_RELATION_RETURN_FUNCTION_NAME_INDEX_TYPE_SEMANTIC_FUNCTION_EXECUTION_INDEX], tempEntity, sameReferenceSet, semanticRelationReturnConnectionDynamic, layer);
							iter1 = (parserForwardPropogationSignalData->semanticRelationReturnFunctionEntityArray).erase(iter1);
							erasedElement = true;
						}
					}	
				...
				{
					cerr << "GIAtxtRelTranslatorParserOperationsClass::generateSemanticRelationsFromTxtRelations{} error: tempEntity->semanticRelationReturnFunctionNameIndexType is illdefined;" << tempEntity->semanticRelationReturnFunctionNameIndexType << endl;
					exit(EXIT_ERROR);
				}
...
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationPair(GIAtranslatorVariablesClass* translatorVariables, const string semanticRelationFunctionName, GIAentityNode* entitySemanticRelationFunction1, GIAentityNode* entitySemanticRelationFunction2, const bool sameReferenceSet, const bool semanticRelationReturnConnectionDynamic, int layer)
{
	#ifdef GIA_DEBUG_TXT_REL_TRANSLATOR_RULES_PRINT_PARSE_PROCESS2
	//GIAtxtRelTranslatorSANIPropagateOperations.printParseTreeDebugIndentation(layer);
...
#ifdef GIA_TXT_REL_TRANSLATOR_RULES_GIA3_USE_SEM_REL_TRANSLATOR_PARSER
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationInMemory(GIAtranslatorVariablesClass* translatorVariables, const string semanticRelationFunctionName, const string semanticRelationFunctionConditionNewName, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, GIAentityNode* entitySemanticRelationFunction1, GIAentityNode* entitySemanticRelationFunction2, GIAentityNode* entitySemanticRelationFunction3, const bool sameReferenceSet, const bool semanticRelationReturnConnectionDynamic)
{
	bool result = true;
	...
	*/
	...
	else if(semanticRelationFunctionName == GIAtxtRelSemanticDependencyRelationNameArray[GIA_TXT_REL_TRANSLATOR_RULES_SEMANTIC_RELATION_conditionNew])
	{
		...
		if(entitySemanticRelationFunction3 != NULL)
		{
			cout << "GIA_TXT_REL_TRANSLATOR_RULES_GIA3_USE_SEM_REL_TRANSLATOR_PARSER: GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationInMemory error: GIA_TXT_REL_TRANSLATOR_RULES_SEMANTIC_RELATION_conditionNew not currently supported; auxiliary tense info should be connected to new condition entity" << endl;
...
#else
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationInNetwork(GIAtranslatorVariablesClass* translatorVariables, const string semanticRelationFunctionName, const string semanticRelationFunctionConditionNewName, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, GIAentityNode* entitySemanticRelationFunction1, GIAentityNode* entitySemanticRelationFunction2, GIAentityNode* entitySemanticRelationFunction3, const bool sameReferenceSet, const bool semanticRelationReturnConnectionDynamic)
{
	bool result = true;
...
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationMultiwordAuxiliary(GIAtranslatorVariablesClass* translatorVariables, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, const bool sameReferenceSet)
{
	bool result = true;
...
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationMultiwordPreposition(GIAtranslatorVariablesClass* translatorVariables, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, const bool sameReferenceSet)
{
	bool result = true;
...
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationMultiwordNoun(GIAtranslatorVariablesClass* translatorVariables, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, const bool sameReferenceSet)
{
	bool result = true;
...
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationMultiwordAlias(GIAtranslatorVariablesClass* translatorVariables, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, const bool sameReferenceSet)
{
	bool result = true;
...
#ifdef GIA_TXT_REL_TRANSLATOR_RULES_CODE_DATE
bool GIAtxtRelTranslatorParserOperationsClass::createSemanticRelationMultiwordDate(GIAtranslatorVariablesClass* translatorVariables, vector<GIAentityNode*>* entitySemanticRelationFunctionListArray, const bool sameReferenceSet)
{
	bool result = true;
