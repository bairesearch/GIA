...
bool GIApreprocessorClass::preprocessTextForGIA(string* inputTextPlainTXTfileName, const string outputLRPTextPlainTXTFileName, const bool isQuery, GIAtranslatorVariablesClass* translatorVariables)
{
	...
	if(!createPreprocessSentencesForGIA(*inputTextPlainTXTfileName, translatorVariables->firstGIApreprocessorSentenceInList))
	{
		result = false;
	}
	...
	#ifdef GIA_PREPROCESSOR_MULTIWORD_REDUCTION
	if(!updateGIApreprocessorMultiwordReductionTagTextCorrespondenceInfo(translatorVariables->firstGIApreprocessorSentenceInList, isQuery, outputLRPTextForNLPonlyPlainTXTFileName))
	{
		result = false;
	}			
	#endif				
...
bool GIApreprocessorClass::createPreprocessSentencesForGIA(const string inputFileName, GIApreprocessorSentence* firstGIApreprocessorSentenceInList)
{
	bool result = true;
	...
	while(charCount < fileContents.length())
	{
		...
		if(endOfSentencePunctuationMarkFound)
		{
			...
			currentGIApreprocessorSentenceInList->sentenceIndexOriginal = sentenceIndex;
			currentGIApreprocessorSentenceInList->sentenceContentsOriginal = sentenceContentsOriginal;
			currentGIApreprocessorSentenceInList->nextTag = new GIApreprocessorSentence();
			currentGIApreprocessorSentenceInList = currentGIApreprocessorSentenceInList->nextTag;
...
void GIApreprocessorClass::generateLogicReferenceSetContentsWithVariableNamesAndAddToSentenceContentsPreprocessedLogicReference(string* sentenceContentsPreprocessedLogicReference, string* sentenceContentsPreprocessedLogicReferenceForNLP, GIApreprocessorLogicReference* currentLogicReferenceInList, int* sentenceIndex)
{
	if(currentLogicReferenceInList->hasSubLogicReference)
	{
		...
		else
		{
			...
			#endif
			#ifdef GIA_PREPROCESSOR_SENTENCE_LOGIC_REFERENCE_OUTPUT_LOGIC_REFERENCE_SETS_FOR_HIGH_LEVEL_SEMANTIC_PARSE_VERBOSE
			currentLogicReferenceInList->logicReferenceVariable->referenceSetSubject->subReferenceSetContents = currentLogicReferenceInList->firstSubLogicReferenceInListGovernor->logicReferenceVariable->logicReferenceVariableName;
			currentLogicReferenceInList->logicReferenceVariable->referenceSetObject->subReferenceSetContents = currentLogicReferenceInList->firstSubLogicReferenceInListDependent->logicReferenceVariable->logicReferenceVariableName;
			logicReferenceSetContentsWithVariableNames = currentLogicReferenceInList->logicReferenceVariable->referenceSetSubject->subReferenceSetContents + STRING_SPACE + currentLogicReferenceInList->logicReferenceVariable->referenceSetDelimiter->subReferenceSetContents + STRING_SPACE + currentLogicReferenceInList->logicReferenceVariable->referenceSetObject->subReferenceSetContents;	//for debug only
			#endif
		}
		#ifdef GIA_PREPROCESSOR_SENTENCE_LOGIC_REFERENCE_OUTPUT_LOGIC_REFERENCE_SETS_FOR_HIGH_LEVEL_SEMANTIC_PARSE_VERBOSE
...
#ifdef GIA_PREPROCESSOR_MULTIWORD_REDUCTION
bool GIApreprocessorClass::updateGIApreprocessorMultiwordReductionTagTextCorrespondenceInfo(GIApreprocessorSentence* firstGIApreprocessorSentenceInList, const bool isQuery, const string outputFileName)
{
	bool result = true;

	//copy sentenceContentsLRPcurrentWord to sentenceContentsLRPforNLPcurrentWord (required by multiword preprocessor)
	GIApreprocessorSentence* currentGIApreprocessorSentenceInList = firstGIApreprocessorSentenceInList;
	while(currentGIApreprocessorSentenceInList->next != NULL)
	{
	...
	while(currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo->next != NULL)
	{
		...
		if(getGIApreprocessorSentence(firstGIApreprocessorSentenceInList, currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo->sentenceIndex, &currentGIApreprocessorSentenceInList))
		{
			...
			if(!getGIApreprocessorReferenceSet(currentGIApreprocessorSentenceInList->firstLogicReferenceInList, entityIndexOriginal, &GIApreprocessorSubReferenceSetFound))
			{
				...
				if(!replaceEntityTagWithNLPonlyTag(currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo, firstGIApreprocessorSentenceInList))
				{
					result = false;
				}
				#ifdef GIA_DEBUG_PREPROCESSOR_SENTENCE_REFERENCE_SET
...
bool GIApreprocessorClass::replaceEntityTagWithNLPonlyTag(GIApreprocessorMultiwordReductionTagTextCorrespondenceInfo* currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo, GIApreprocessorSentence* firstGIApreprocessorSentenceInList)
{
	bool result = false;
	GIApreprocessorSentence* currentGIApreprocessorSentenceInList = firstGIApreprocessorSentenceInList;
	while(currentGIApreprocessorSentenceInList->next != NULL)
		...
		if(currentGIApreprocessorSentenceInList->sentenceIndexOriginal == currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo->sentenceIndex)
		{
			...
			while(currentTagInPlainTextSentence->nextTag != NULL)
			{
				if(currentTagInPlainTextSentence->entityIndex == currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo->entityIndex)
				{
					result = true;
					#ifdef GIA_DEBUG_PREPROCESSOR_SENTENCE_REFERENCE_SET
					cout << "GIApreprocessorClass::replaceEntityTagWithNLPonlyTag; currentTagInPlainTextSentence->tagName  = " << currentTagInPlainTextSentence->tagName << endl;
					...
					#endif
					currentTagInPlainTextSentence->tagName = currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo->wordWithLRPforNLPonly;
				}
