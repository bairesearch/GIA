...
bool GIApreprocessorClass::preprocessTextForGIA(string* inputTextPlainTXTfileName, const string outputLRPTextPlainTXTFileName, const bool isQuery, GIAtranslatorVariablesClass* translatorVariables)
{
	...
	if(!GIApreprocessorMultiwordReduction.parseTextFileAndReduceLanguage(*inputTextPlainTXTfileName, translatorVariables->firstGIApreprocessorSentenceInList, outputLRPTextPlainTXTFileNameIntermediaryMultiword, outputLRPTextForNLPonlyPlainTXTFileNameIntermediaryMultiword))
	{
		result = false;
	}
	#endif
...
bool GIApreprocessorClass::createPreprocessSentencesForGIA(const string inputFileName, GIApreprocessorSentence* firstGIApreprocessorSentenceInList)
{
	...
	while(charCount < fileContents.length())
	{
		...
		if(endOfSentencePunctuationMarkFound)
		{
			...
			if(!createGIApreprocessorSentence(currentGIApreprocessorSentenceInList, &sentenceContentsOriginal, sentenceIndexOriginal))
			{
				result = false;
			}
...
bool GIApreprocessorClass::createGIApreprocessorSentence(GIApreprocessorSentence* currentGIApreprocessorSentenceInList, const string* sentenceContentsOriginal, int sentenceIndexOriginal)
{
	bool result = true;
...
bool GIApreprocessorClass::replaceEntityTagWithNLPonlyTag(GIApreprocessorMultiwordReductionTagTextCorrespondenceInfo* currentLRPtoLRPforNLPonlyTagNameAndLocationCorrespondenceInfo, GIApreprocessorMultiwordReductionSentence* firstTagInPlainText)
{
	bool result = false;
	...
	while(currentTagInPlainText->nextSentence != NULL)
	{
		...
		{
			GIApreprocessorMultiwordReductionWord* currentTagInPlainTextSentence = currentTagInPlainText->firstTagInSentence;
