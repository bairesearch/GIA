		...
		*/
		...
		{	
			...
			*/		
			...
			currentReferenceInPrintList = createBox(currentReferenceInPrintList, &pos1, GIA_DRAW_ACTION_NODE_WIDTH, GIA_DRAW_ACTION_NODE_HEIGHT, entityColour, &(entityNode->entityName), writeFileObject);
	...
	if(!(actionNode->initialisedForPrinting))
	{
		...
		*/
		...
		{										
			...
			currentReferenceInPrintList = createBox(currentReferenceInPrintList, &pos1, GIA_DRAW_ACTION_NODE_WIDTH, GIA_DRAW_ACTION_NODE_HEIGHT, GIA_DRAW_ACTION_NODE_COLOUR, &(actionNode->actionName), writeFileObject);
		}
...
Reference * initialiseConditionNodeForPrinting(GIASharedConditionNode * sharedConditionNode, int y, int x, int initialiseOrPrint, Reference * currentReferenceInPrintList, ofstream * writeFileObject)
{
	...
	if(!(sharedConditionNode->initialisedForPrinting))
	{
		...
		{	
			...
			{
				currentReferenceInPrintList = initialiseActionNodeForPrinting(sharedConditionNode->parentAction, y-DRAW_Y_SPACE_BETWEEN_ENTITIES, x, initialiseOrPrint, currentReferenceInPrintList, writeFileObject);
		...
		else
		{
			...
			{
				currentReferenceInPrintList = initialiseEntityNodeForPrinting(sharedConditionNode->parentProperty, y-DRAW_Y_SPACE_BETWEEN_ENTITIES, x, initialiseOrPrint, currentReferenceInPrintList, writeFileObject);
		...
		if(sharedConditionNode->conditionIsAction)
		{
			...
			{
				currentReferenceInPrintList = initialiseActionNodeForPrinting(sharedConditionNode->conditionAction, y, x+DRAW_X_SPACE_BETWEEN_ENTITIES, initialiseOrPrint, currentReferenceInPrintList, writeFileObject);
		...
		else
		{
			...
			if(sharedConditionNode->conditionEntity != NULL)
			{
				...
				currentReferenceInPrintList = initialiseEntityNodeForPrinting(sharedConditionNode->conditionEntity, y, x+DRAW_X_SPACE_BETWEEN_ENTITIES, initialiseOrPrint, currentReferenceInPrintList, writeFileObject);
...
Reference * createBox(Reference * currentReferenceInPrintList, vec * vect, double width, double height, int colour, string * text, ofstream * writeFileObject)
{
	...
	*/
	...
	writeSVGBox(writeFileObject, &positionSVG, width, height, colour, GIA_FILE_TEXT_BOX_OUTLINE_WIDTH_SVG);
