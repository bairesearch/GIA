...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::applyAdvancedFeatures(GIAtranslatorVariablesClass* translatorVariables)
{
...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::extractDates(GIAtranslatorVariablesClass* translatorVariables)
{
	#ifdef GIA_RELEX
...
#ifdef GIA_RELEX
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::extractDatesRelex(GIAtranslatorVariablesClass* translatorVariables)
{
	for(int i=0; i<GIAsentenceClass.getMaxIndexOfDynamicallyGeneratedEntity(translatorVariables->currentSentenceInList); i++)
	{
		if((*translatorVariables->GIAentityNodeArrayFilled)[i])
	...
	while(currentRelationInList->next != NULL)
	{
		...
		{
		#endif
			if((currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_DATE_DAY) || (currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_DATE_YEAR))
			{
						...
						if(currentEntity->conditionType == CONDITION_NODE_TYPE_TIME)
						{
							...
							if(timeConditionEntity->entityName == currentRelationInList->relationGovernor)
							{
								if(timeEntity->timeConditionNode != NULL)
								{
									if(!(timeEntity->timeConditionNode->tenseOnlyTimeCondition))
									{
										if(currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_DATE_DAY)
										{
										...
										if(currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_DATE_YEAR)
										{
											GIAtranslatorOperations.disableInstanceAndNetworkIndexEntityBasedUponFirstSentenceToAppearInNetwork((*translatorVariables->GIAentityNodeArray)[currentRelationInList->relationDependentIndex]);
			...
			if(currentEntity->conditionType == CONDITION_NODE_TYPE_TIME)
			{
				if(timeEntity->timeConditionNode != NULL)
				{
					if(!(timeEntity->timeConditionNode->tenseOnlyTimeCondition))
					{
						...
						int64_t timeConditionTotalTimeInSeconds = calculateTotalTimeInSeconds(timeConditionEntity->timeConditionNode->dayOfMonth, timeConditionEntity->timeConditionNode->month, timeConditionEntity->timeConditionNode->year);
...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::addTimeToSubstance(GIAentityNode* timeConditionEntity)
{
...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::extractQuantities(GIAtranslatorVariablesClass* translatorVariables)
{
	/*
...
#ifdef GIA_STANFORD_CORENLP
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::extractQuantitiesStanfordCoreNLP(GIAtranslatorVariablesClass* translatorVariables)
{
	GIArelation* currentRelationInList = translatorVariables->currentSentenceInList->firstRelationInList;

	#ifdef GIA_SYN_REL_TRANSLATOR_CONVERT_AMOD_WITH_NUMBERS_TO_QUANTITY_RELATION
	...
	while(currentRelationInList->next != NULL)
	{
		...
		{
		#endif
			if(currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_ADJECTIVE_AMOD)
			{
				string s = currentRelationInList->relationDependent;
				int value = SHAREDvars.convertStringToInt(s);
				if(value || (s == "0"))
				{
					currentRelationInList->relationType = GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY;
				}
			}
		#ifdef GIA_SYN_REL_TRANSLATOR_DO_NOT_PARSE_DISABLED_RELATIONS_OLD
		}
		...
		if(!(currentRelationInList->disabled))
		{
			...
			if(currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY)
			{
				...
				if(quantityEntity->NERTemp != FEATURE_NER_DATE)		//do not assume quantity entities when dealing with Stanford Dates (as they have already been parsed).
				{
					...
					GIAsemRelTranslatorOperations.GIA2nonHeuristicImplementationGenerateExperiencesForConnectionistNetworkTrain(translatorVariables, GIA_ENTITY_VECTOR_CONNECTION_TYPE_QUANTITY, currentRelationInList->relationGovernorIndex, currentRelationInList->relationDependentIndex, false);
					#endif
					...
					while(currentRelationInList2->next != NULL)
					{
						...
						{
						#endif
							if(currentRelationInList2->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY_MOD)
							{
								if((currentRelationInList2->relationGovernor == currentRelationInList->relationDependent) || (currentRelationInList2->relationGovernor == currentRelationInList->relationGovernor))	//OLD before GIA 2b7d/2c2b: if(currentRelationInList2->relationGovernor ==currentRelationInList->relationGovernor)
								{
									...
									*/
									...
									//GIAsynRelTranslatorGeneric.addRelationshipArtificialToEntityAndGenerateSemanticRelations(entityNode, relationshipObjectEntity, sameReferenceSet, GIA_ENTITY_TYPE_CONDITION, conditionName, translatorVariables, false, currentRelationInList->relationGovernorIndex, currentRelationInList2->relationDependentIndex);	//GIA semantic parser output is unsupported for artifical conditions
...
#ifdef GIA_RELEX
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::extractQuantitiesRelex(GIAtranslatorVariablesClass* translatorVariables)
{
	GIArelation* currentRelationInList = translatorVariables->currentSentenceInList->firstRelationInList;
	while(currentRelationInList->next != NULL)
	{
			...
			if(currentRelationInList->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY)
			{
				...
				while(currentRelationInList2->next != NULL)
				{
					...
					if(!(currentRelationInList2->disabled))
					{
						...
						if(currentRelationInList2->relationType == GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY_MULT)
						{
							if(currentRelationInList2->relationGovernor == currentRelationInList->relationDependent)
							{
								#ifdef GIA_SEM_REL_TRANSLATOR_GENERATE_EXPERIENCES_FOR_CONNECTIONIST_NETWORK_TRAIN
								//quantity multipliers not currently supported by GIA2 [NB Stanford CoreNLP does not use them, only Relex]
								#endif
				...
				for(int i=0; i<GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_QUANTITY_ARGUMENT_IMPLY_MEASURE_PER_NUMBER_OF_TYPES; i++)
				{
				...
				{//eg "every hour" or "every day" - convert to measure_per system
					...
					if(foundQuantityOwner)
					{
						...
						string conditionName = GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_MEASURE_PER;
						//GIAsynRelTranslatorGeneric.addRelationshipArtificialToEntityAndGenerateSemanticRelations(newQuantityTimesEntity, measureSubstance, sameReferenceSet, GIA_ENTITY_TYPE_CONDITION, conditionName, translatorVariables, false, FEATURE_INDEX_OF_QUANTITY_TIMES_UNKNOWN, currentRelationInList->relationGovernorIndex);	//this (conditionRelationshipSubjectEntity<->condition connection) is not currently been generated correctly by GIA1	//GIA semantic parser output is unsupported for artifical conditions
...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::defineTenseOnlyTimeConditions(GIAtranslatorVariablesClass* translatorVariables)
{
...
void GIAsynRelTranslatorApplyAdvancedFeaturesClass::defineActionConcepts1(GIAtranslatorVariablesClass* translatorVariables)
{
	...
	param.useRelationTest[REL1][REL_ENT3] = true; param.relationTest[REL1][REL_ENT3] = GIA_SYN_REL_TRANSLATOR_RELATION_TYPE_MODAL_AUX;
	...
	GIAsynRelTranslatorGeneric.genericDependecyRelationInterpretation(&param, REL1);

}

void GIAsynRelTranslatorApplyAdvancedFeaturesClass::defineActionConcepts2(GIAtranslatorVariablesClass* translatorVariables)
{
	/*
