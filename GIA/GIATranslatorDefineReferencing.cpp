				...
				{//continue only for references, eg pronoun (not for their source, eg name)	
					...
					if(currentMentionInList->sentence == currentSentenceInList->sentenceIndex)
					{
						...
						if(GIAEntityNodeArrayFilled[currentSentenceEntityNodeIndex])
						{
							...
							{
								#ifdef GIA_TRANSLATOR_DEBUG
								cout << "referenceSourceHasBeenFound: assigning " << GIAConceptNodeArray[currentSentenceEntityNodeIndex]->entityName << " to " << referenceSource->entityName << "." << endl;
								#endif
								...
								GIAConceptNodeArray[currentSentenceEntityNodeIndex] = referenceSource;
